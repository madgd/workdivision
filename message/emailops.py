...
class EmailSender:
    ...
    """
    smtp = None
    def __init__(self, conf_sec=ConfigSec):
        # config
        config = configparser.ConfigParser()
        config.read(EmailConfig)
        if not conf_sec:
            conf_sec = ConfigSec
        self.host_server = config.get(conf_sec, "host_server")
        self.fromaddress = config.get(conf_sec, "fromaddress")
        self.password = config.get(conf_sec, "password")
        self.inport = config.get(conf_sec, "inport")
        try:
...
def sendEmail(addresses, content, mail_title, attach, cc=[], conf_sec=ConfigSec):
    """
    ...
    if not conf_sec:
        conf_sec = ConfigSec
    host_server = config.get(conf_sec, "host_server")
    fromaddress = config.get(conf_sec, "fromaddress")
    password = config.get(conf_sec, "password")
    inport = config.get(conf_sec, "inport")
...
def receiveEmail3(conf_sec=ConfigSec, **kwargs):
    # config
    config = configparser.ConfigParser()
    config.read(EmailConfig)
    if not conf_sec:
        conf_sec = ConfigSec
    email = config.get(conf_sec, "fromaddress")
    password = config.get(conf_sec, "password")
    server = config.get(conf_sec, "host_server")
...
if __name__ == '__main__':
    ...
    # sender = EmailSender('DEFAULT')
    # for i in range(1):
    #     sender.sendEmail(addresses, content, Subject, attach, cc)
    #     time.sleep(1)
    # receiveEmail()
    # receiveEmail2()
    res = receiveEmail3('gszl1', uid__range='2577703055:*')
    for uid, message in res:
        print(uid)
